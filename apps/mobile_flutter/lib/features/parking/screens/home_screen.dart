...
class HomeScreen extends StatelessWidget {
  ...
  static const int _parkingFee = 3500;
  ...
  Future<void> _processDirectPayment({
    ...
    final durationMinutes = exitAt.difference(entryAt).inMinutes.clamp(0, 1000000).toInt();
    ...
    try {
      ...
      final providerUserId = Provider.of<UserProvider>(parentContext, listen: false).userId;
      final userId = providerUserId.isNotEmpty ? providerUserId : prefs.getString('userId') ?? '';
      final vehicles = Provider.of<VehicleProvider>(parentContext, listen: false).vehicles;
      final matchedVehicles = vehicles.where((vehicle) => vehicle.plateNumber == currentVehicle);
      final matchedVehicle = matchedVehicles.isEmpty ? null : matchedVehicles.first;
      ...
      SharedData.parkingEntryAt.value = DateTime.now();

      messenger.showSnackBar(
        SnackBar(
          content: Text('✅ $cardName(으)로 [$currentVehicle] ${_formatDuration(durationMinutes)} 이용료 $_parkingFee원이 정산되었습니다.'),
        ...
        child: Column(
          ...
          children: [
            ...
            const Padding(
              ...
              child: Text('빠른 바로가기', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey)),
            ),
            ...
            Center(
              child: TextButton.icon(
                onPressed: () {
                  ...
                  if (SharedData.isCurrentlyParked.value) {
                    SharedData.parkingEntryAt.value = DateTime.now();
                  }
                },
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      Text('입차 시간: ${_formatClock(entryAt)}', style: const TextStyle(color: Colors.white70, fontSize: 13)),
                    ],
                  ),
                  const Row(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      ...
                      Text('3,500원', style: TextStyle(color: Colors.yellowAccent, fontSize: 22, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ],
              );
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '[$currentVehicle] 차량의 주차 요금 ${_formatWon(_parkingFee)}을 정산합니다.',
                ...
                if (hasPaymentMethod) ...[
                  ...
                  ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      minimumSize: const Size(double.infinity, 56),
                      backgroundColor:
                          isDarkMode ? Colors.grey.shade700 : Colors.black87,
                      foregroundColor: Colors.white,
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(16),
                      ),
  ...
  Widget _buildServiceIcon({
    ...
    final bgColor = isDarkMode ? Colors.white.withValues(alpha: 0.15) : Theme.of(context).primaryColor.withValues(alpha: 0.1);
  ...
  Future<void> _syncPaymentMethodFromStorage() async {
    ...
    final methods = decoded.whereType<Map>().map((method) => Map<String, dynamic>.from(method)).toList();
    final mainMethod = methods.firstWhere(
      (method) => method['id'] == savedMainId,
      orElse: () => methods.first,
    );
    SharedData.paymentMethod.value = mainMethod['name']?.toString() ?? '등록된 결제 수단 없음';
  }
