...
class _PaymentMethodScreenState extends State<PaymentMethodScreen> {
  ...
  static const String _portOneUserCode = 'imp68124833';
  ...
  Future<void> _loadSavedPaymentMethods() async {
    ...
    if (savedMethods != null) {
      setState(() {
        ...
        registeredMethods = List<Map<String, dynamic>>.from(
          jsonDecode(savedMethods),
        );
  ...
  Future<void> _addPaymentMethod(
    ...
    final String resolvedBillingKey =
        billingKey ??
        customerUid ??
        impUid ??
        "test_billing_key_${DateTime.now().millisecondsSinceEpoch}";
    ...
    try {
      ...
      setState(() {
        ...
        registeredMethods.add({
          ...
          'type': type,
          'billingKey': resolvedBillingKey,
          if (pgProvider != null) 'pgProvider': pgProvider,
          if (payMethod != null) 'payMethod': payMethod,
          if (impUid != null) 'impUid': impUid,
          if (merchantUid != null) 'merchantUid': merchantUid,
  ...
  Future<void> _startPortOnePayment({
    ...
    final merchantUid = 'aupark_card_${DateTime.now().millisecondsSinceEpoch}';
    final customerUid = 'aupark_customer_$userId';
      ...
      MaterialPageRoute(
        builder: (context) => IamportPayment(
          ...
          data: PaymentData(
            ...
            merchantUid: merchantUid,
            amount: 100,
            customerUid: isBilling ? customerUid : null,
            buyerName: 'Au-Park 사용자',
            buyerTel: '010-1234-5678',
            buyerEmail: 'test@aupark.local',
            appScheme: 'aupark',
          ),
          callback: (Map<String, String> result) async {
            ...
            if (result['imp_success'] == 'true') {
              ...
              final resultMerchantUid = result['merchant_uid'] ?? merchantUid;
              final displayName = '$pgName 테스트 카드';
              await _addPaymentMethod(
                displayName,
                'card',
                billingKey: isBilling
                    ? customerUid
                    : impUid ?? resultMerchantUid,
                pgProvider: pgCode,
                payMethod: payMethod,
                impUid: impUid,
                merchantUid: resultMerchantUid,
                customerUid: isBilling ? customerUid : null,
                status: 'paid',
              );
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    '❌ 결제 실패: ${result['error_msg'] ?? '오류가 발생하였습니다. 관리자에게 문의 바랍니다.'}',
                  ),
                  backgroundColor: Colors.red,
                  behavior: SnackBarBehavior.floating,
                ),
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              '출차 시 주차 요금이 주 결제 수단으로 자동 결제됩니다.',
            ...
            Container(
              ...
              child: const Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '등록하신 결제 수단을 통해 자동 정산 시스템 이용이 가능합니다.',
                      style: TextStyle(
                        fontSize: 12,
                        color: Colors.grey,
                        height: 1.5,
